...
class PackagesDialog(DialogBase):
    ...
    def setup(self, worker, output, error):
        ...
        if error:
            ...
            modified = actions_link_names.intersection(actions_unlink_names)
            modified_count = len(modified)
            plural_total_modified = 's' if modified_count != 1 else ''

            count_total_packages = len(packages) + len(deps) - modified_count
            ...
            if actions_link and actions_unlink:
                description = '{0} package{2} will be removed ' \
                              '{1} package{3} will be modified'.format(
                    count_total_packages, modified_count, plural_total, plural_total_modified
                )
                self.table.showColumn(2)
                self.table.showColumn(3)

            elif actions_link and not actions_unlink:
