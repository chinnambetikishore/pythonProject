...
from anaconda_navigator.config import (CHANNELS_PATH, CONF, DEFAULT_BRAND,
                                       LINUX, MAC, WIN, DEFAULT_ANACONDA_MAIN_URL)
...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.current_prefix = self.config.get(
            'main', 'default_env', self.api.ROOT_PREFIX
        )
        ...
        if tab_environments:
            ...
            self.sig_logged_in.connect(
                lambda: self.tab_environments.sig_update_index_requested.emit('Login')
            )
            self.sig_logged_out.connect(
                lambda: self.tab_environments.sig_update_index_requested.emit('Logout')
            )
        # if tab_project:
    ...
    def update_login_status(self, user_data=None):
        ...
        def _update_tool_tip(worker, output, error):
            ...
            if worker.username:
                url = "{0}/{1}".format(self._anaconda_url, worker.username)
                try:
    ...
    def show_preferences(self):
        ...
        self._dialog_preferences = PreferencesDialog(
            parent=self, config=self.config, environments=self.environments
        )
        ...
        if dlg.exec_():
            ...
            if selected_filter == dlg.CONDA_SPEC_FILES:
                ...
                if selected_filter == dlg.CONDA_ENV_FILES:
                    ...
                    ls = raw.split('\n')
                    deps = [
                        line for line in ls if line and not line.strip().startswith('#')
                    ]
                    data = {
                        'name': name,
                        'dependencies': ['python', {
        ...
        if app_type == AppType.INSTALLABLE:
            ...
            def _finished(worker, output, error):
                self.launch_application_2(
                    package_name, command, leave_path_alone, prefix, non_conda
                )
...
#        self.check_dependencies_before_install(worker, worker.output, error)
    ...
    # --- VSCode
    # -------------------------------------------------------------------------
    def _output_external_app_ready(self, worker, output, error):
        """Callback for handling external app actions."""
        if not isinstance(output, dict):
            output = {}
        error_text = output.get('error', '')
        exception_type = output.get('exception_type', '')
    ...
    def _partial_external_app_ready(self, worker, output, error, app_name):
        """Callback for handling external app actions."""
        if isinstance(output, dict):
            ...
            if total and progress and app_name in path:
                self.update_status(
                    'Installing <b>{}<b>'.format(app_name),
                    'Downloading...',
                    value=progress,
                    max_value=total,
                )
